...
public abstract class WOIMVerifier implements AuthHandler.Verifier {
    ...
    @Override
    public final void handleAuthFailed(Channel channel, Packet packet, VerifyException e) {
        channel.writeAndFlush(PayloadUtil.buildPayload(new Response(ErrorCode.CODE_LOG_IN_ERR,ErrorCode.MESSAGE_LOG_IN_ERR,e.getMessage()),
                Payload.Cmd.RESPONSE,packet.getSn()));
    }
